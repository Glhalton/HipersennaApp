...
export default function ValidityForm() {
  ...
  useEffect(() => {
    if (!cameraModal && scanned && codProductInput.length > 0) {
      productSearch("codauxiliar", codProductInput, 1);
          ...
          <View>
            <Input
              ...
              onChangeText={(text) => {
                ...
                // evita erro quando estiver vazio
                if (clean === "") {
                  setProductPrice("");
                  return;
                }

                // adiciona vírgula antes dos centavos
                clean = (Number(clean) / 100).toFixed(2).replace(".", ",");

                setProductPrice(`R$ ${clean}`);
              }}
              value={productPrice}
            />
          </View>
